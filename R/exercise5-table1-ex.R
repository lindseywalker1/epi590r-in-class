...
tbl_summary(
	...
	), #statistics is still part of a funciton, that needs to be in the original parentheses
	statistic = list(
		income ~ "10th {p10}, 90th {p90}",
		starts_with("sleep") ~ "min = {min}; max = {max}"
	), #digits is still part of a funciton, that needs to be in the original parentheses
	digits = list(
		income ~ c(3,3),
