...
# here we made models, and now can just specify model name
...
## Table comparing the models with (model 1) and without interaction (model 2)
...
tbl_uvregression(
	...
	label = list(
		...
		income ~ "Income",
		nsibs ~ "# of Siblings"))
# Dr. Louisa says she likes the horizontal look of this table better
# for that you could do individual uvregressions and then table merge them together
# ex: tbl_sleep_wkdy_sex < tbl_uvregression(
	#nlsy,
	# y= sleep_wkdy,
	# include = sex_cat,
	#method = lm)
#tbl_merge(list(tbl_sleep_wkdy_sex AND OTHER SEPARATE TABLES),
					#tab_spanner = c("**Table 1 name**", "**Table 2 name**"))

#Fit a Poisson regression (family = poisson()) for the number of siblings,
#using at least 3 predictors of your choice.
#Create a nice table displaying your Poisson regression and its exponentiated coefficients.

poisson.model <- glm(nsibs ~ sleep_wkdy + income + sleep_wknd,
...
#Instead of odds ratios for wearing glasses, as in the example,
#we want risk ratios. We can do this by specifying
#in the regression family = binomial(link = "log").
#Regress glasses on eyesight_cat sex_cat and create a table showing
#the risk ratios and confidence intervals from this regression.
...
#Since family = binomial(link = "log") often doesn’t converge, we often
...
poisson.model_rr <- glm(glasses ~ eyesight_cat + sex_cat,
